...
| 阶段6.5/6.6 统一 TTS | ✅ 代码完成，prod 5 句付费小样通过 | `src/mmm/tts/` 适配层；dry 固定 Edge，prod 默认 MiniMax；**full_then_split**（完整合成→WordBoundary/subtitle 时间轴→切回 tts_XXX.wav）；LLM 表演计划只做发音/停顿/语气/情绪标注，禁止改稿；闸口3 计划 HTML + plan_sha256 显式审批；render/jianying 复用片段缓存；修复 MiniMax 语气标签、发音替换时间轴与毫秒切分换算；`tts-prod-smoke` 5 段实切通过；待办：最终音色试听、长稿边界大样 |
